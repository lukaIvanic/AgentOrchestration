...
def cli():
    ...
    if args.command == "init":
        ...
        if args.dry_run:
            print(f"Dry-run valid manifest: {args.manifest}")
            print(f"Manifest keys: {', '.join(sorted(manifest.keys()))}")
            return
        _deploy_agent(args.manifest, manifest)
...
def _load_manifest(path: str) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"Manifest is invalid YAML: {exc}")

    if not isinstance(manifest, dict):
        raise SystemExit("Manifest must be a mapping/object")
    return manifest


def _deploy_agent(path: str, manifest: dict) -> None:
    print(f"Deploying agent from manifest: {path}")
