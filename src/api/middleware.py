...
from .auth import AuthService, AuthenticationError, PermissionDenied
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if self._requires_auth(request.url.path):
            try:
                principal = self.auth_service.authenticate_request(request)
    ...
    def _requires_auth(self, path: str) -> bool:
        if not path.startswith("/api/v2"):
            return False
        return path.rstrip("/") != "/api/v2/auth/token"
